...
from src.common.config import CATASTRO_DIR, PROCESSED_DATA_DIR, get_yolo_dataset_settings
from src.common.constants import ROOF_CLASS_ID, ROOF_CLASS_NAME
...
def _polygon_lines(geometry, transform: rasterio.Affine, window: Window) -> list[str]:
    ...
    for polygon in polygons:
        ...
        if len(coordinates) >= 6:
            ...
            lines.append(f"{ROOF_CLASS_ID} {values}")
    return lines


def write_data_yaml(output_dir: Path) -> Path:
    """Write Ultralytics YOLO data.yml pointing at train/images."""
    dataset_root = output_dir.resolve()
    yaml_path = dataset_root / DATA_YAML_NAME
    content = (
        ...
        f"nc: 1\n"
        f"names:\n"
        f"  {ROOF_CLASS_ID}: {ROOF_CLASS_NAME}\n"
    )
...
def prepare_dataset(
    ...
    print(f"Finished: {processed} YOLO samples in {train_images_dir}")
    return processed


def main() -> None:
    defaults = get_yolo_dataset_settings()
    parser = argparse.ArgumentParser(
        description="Create YOLO-seg train split and data.yml from Catastro building crops"
    )
    ...
    parser.add_argument(
        "--offset-meters",
        type=float,
        default=defaults["offset_meters"],
        help=(
            "Context added around the Catastro building on every side "
            f"(default from config: {defaults['offset_meters']})"
        ),
    )
